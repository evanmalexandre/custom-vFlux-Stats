    ...
    foreach ($vm in $VMs) {
      ...
      foreach ($stat in $stats) {
        ...
        If(-Not($instance) -or ($instance -eq '')) {
          ...
          $InfluxStruct.MetricsString += ('{0},host={1},type={2},vc={3},cluster={4},unit={5},interval={6},numcpu={7},memorygb={8},memoryusage={9} value={10} {11}' -f $measurement, $name, $type, $vc, $cluster, $Unit, $interval, $numcpu, $memorygb, $memoryusage, $value, $timestamp)
          $InfluxStruct.MetricsString += "`n"
        }
        Else {
          #return instance (i.e. cpucores, vmnics, etc.)
          $InfluxStruct.MetricsString = ''
          $InfluxStruct.MetricsString += ('{0},host={1},type={2},vc={3},cluster={4},unit={5},interval={6},instance={7},numcpu={8},memorygb={9},memoryusage={10} value={11} {12}' -f $measurement, $name, $type, $vc, $cluster, $Unit, $interval, $instance, $numcpu, $memorygb, $memoryusage, $value, $timestamp)
          $InfluxStruct.MetricsString += "`n"
        }

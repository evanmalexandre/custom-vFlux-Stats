...
Begin {
  ...
  $InfluxStruct = New-Object -TypeName PSObject -Property @{
    ...
    InfluxDbName               = 'vmstorage'                                        #to follow my examples, set to 'compute' here and run "CREATE DATABASE compute" from Influx CLI
  ...
  ## stat preferences
  $VmStatTypes  = 'disk.usage.average'
  ...
  ## Iterate through VM list
    foreach ($vm in $VMs) {
      ...
      foreach ($stat in $stats) {
        ...
        $measurement = "disk.usage.average" ## previously $stat.Metric
        $value = $vm.UsedSpaceGB
        ...
        If(-Not($instance) -or ($instance -eq '')) {
          ...
          $InfluxStruct.MetricsString += ('{0},host={1},type={2},vc={3},cluster={4} value={5} {6}' -f $measurement, $name, $type, $vc, $cluster, $value, $timestamp)
          $InfluxStruct.MetricsString += "`n"
        }
        Else {
          #return instance (i.e. cpucores, vmnics, etc.)
          $InfluxStruct.MetricsString = ''
          $InfluxStruct.MetricsString += ('{0},host={1},type={2},vc={3},cluster={4],instance={5} value={6} {7}' -f $measurement, $name, $type, $vc, $cluster, $instance, $value, $timestamp)
          $InfluxStruct.MetricsString += "`n"
        }
